...
public void MercsDieTwice_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		MercsDieTwice_ApplyPerk(client, perk);
	}
	else
	{
		MercsDieTwice_RemovePerk(client, reason);
	}
}
