...
public void Explode_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		Explode_ApplyPerk(client, perk);
	}
	else
	{
		Explode_RemovePerk(client, reason);
	}
}
