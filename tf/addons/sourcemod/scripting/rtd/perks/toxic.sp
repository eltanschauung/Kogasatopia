...
public void Toxic_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		Toxic_ApplyPerk(client, perk);
	}
	else
	{
		Toxic_RemovePerk(client, reason);
	}
}
