...
public void TeamAutoheal_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		TeamAutoheal_ApplyPerk(client, perk);
	}
	else
	{
		TeamAutoheal_RemovePerk(client, reason);
	}
}
